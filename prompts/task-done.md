...
If nothing is ready, the response reports unfinished work and owners, or confirms the list is complete. An empty group still counts as unfinished. Missing IDs and invalid completions return an error without completing a task. task_update completion remains available for compatibility, but does not return a handoff.
